...
export type TreeRenderHooks = {
    setTreeItemEditingState(treeItem: HTMLElement | null, input: HTMLInputElement | null, isActive: boolean): void;
...
async function collectSiblingNames(parentHandle: FileSystemDirectoryHandle) {
    ...
    for await (const entry of parentHandle.entries()) {
        const [name] = entry as [string, FileSystemFileHandle | FileSystemDirectoryHandle];
        names.add(name);
    }
...
async function copyDirectoryContents(sourceHandle: FileSystemDirectoryHandle, targetHandle: FileSystemDirectoryHandle) {
    for await (const entry of sourceHandle.entries()) {
        const [name, handle] = entry as [string, FileSystemFileHandle | FileSystemDirectoryHandle];

        if (handle.kind === 'directory') {
            const childDirectoryHandle = await targetHandle.getDirectoryHandle(name, { create: true });
...
export async function moveDirectory(
    ...
    const newHandle = await toParent.getDirectoryHandle(targetName, { create: true });
...
export async function renameMarkdownFile(
    ...
    const normalizedName = await makeUniqueMarkdownFilename(parentHandle, requestedName, oldName);
    ...
    const file = await fileHandle.getFile();
    const content = await file.text();
    const newHandle = await parentHandle.getFileHandle(normalizedName, { create: true });
    const writable = await newHandle.createWritable();
    await writable.write(content);
    await writable.close();
...
export async function renameDirectory(
    ...
    const normalizedName = await makeUniqueDirectoryName(parentHandle, requestedName, oldName);
    if (normalizedName === oldName) {
        return { directoryHandle, name: oldName };
    }

    const newHandle = await parentHandle.getDirectoryHandle(normalizedName, { create: true });
...
export async function getTree(dirHandle: FileSystemDirectoryHandle): Promise<TreeEntry[]> {
    ...
    for await (const entry of dirHandle.entries()) {
        ...
        if (handle.kind === 'directory') {
            ...
            items.push({ name, kind: 'directory', handle, parentHandle: dirHandle, children });
...
export function setTreeItemEditingState(treeItem: HTMLElement | null, input: HTMLInputElement | null, isActive: boolean) {
    if (!input) return;
...
export function hasExpandedFolders(container: HTMLElement | null) {
    ...
    const folders = Array.from(container.querySelectorAll<HTMLElement>('.tree-item[aria-expanded]'));
    return folders.some((folder) => folder.getAttribute('aria-expanded') === 'true');
}

export function setAllFoldersExpanded(container: HTMLElement | null, isExpanded: boolean) {
    if (!container) return;

    const folders = Array.from(container.querySelectorAll<HTMLElement>('.tree-item[aria-expanded]'));
    for (const folder of folders) {
        const childrenContainer = folder.querySelector('.children') as HTMLElement | null;
        ...
        caret.textContent = isExpanded ? 'keyboard_arrow_down' : 'keyboard_arrow_right';
